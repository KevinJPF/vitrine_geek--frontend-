...
describe("Gerenciamento de Pedidos - Administrador", () => {
  beforeEach(() => {
    ...
    cy.intercept("GET", "**/meus-pedidos").as("getMeusPedidos");
    cy.intercept("PATCH", "**/pedidos/*").as("patchPedido");
    cy.intercept("PATCH", "**/meus-pedidos/*").as("patchMeuPedido");
  });
  ...
  it("Deve completar o fluxo completo: EM_PROCESSAMENTO -> APROVADA -> EM_TRANSPORTE -> ENTREGUE", () => {
    ...
    // Encontrar um pedido com status EM_PROCESSAMENTO
    cy.get('[data-cy^="pedido-status-"]').each(($el, index) => {
      if ($el.text().includes("Em Processamento")) {
        ...
        cy.get('[data-cy="dropdown-novo-status-pedido"]').click({ delay: 500 });
        cy.get('[data-cy="dropdown-status-em_transporte"]').click({
          delay: 500,
        });
        cy.get('[data-cy="btn-salvar-status-pedido"]').click({ delay: 500 });
        cy.wait("@patchPedido");
  ...
  it("Deve solicitar uma troca de pedido", () => {
    cy.visit("http://localhost:5173/meus-pedidos");
    cy.wait("@getMeusPedidos");

    // Encontrar um pedido com status ENTREGUE
    cy.get('[data-cy^="pedido-status-"]').each(($el, index) => {
      if ($el.text().includes("Entregue")) {
        // Clicar em gerenciar no primeiro pedido
        cy.get(`[data-cy="btn-trocar-pedido-${index}"]`).click();

        // Selecionar produto para troca
        cy.get(`[data-cy="btn-produto-pedido-0"]`).click();
  ...
  it("Deve cancelar a alteração de status sem salvar", () => {
    ...
    cy.get('[data-cy^="pedido-status-"]').each(($el, index) => {
      if ($el.text().includes("Em Processamento")) {
        ...
        cy.get(`[data-cy="pedido-status-${index}"]`)
          .invoke("text")
          .then((statusInicial) => {
            // Abrir modal
            cy.get(`[data-cy="btn-gerenciar-pedido-${index}"]`).click({
            ...
            // Cancelar
            cy.get('[data-cy="btn-cancelar-status-pedido"]').click({
              delay: 500,
            });
